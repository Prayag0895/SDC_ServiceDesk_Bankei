...
class TicketSerializer(serializers.ModelSerializer):
    ...
    def validate_attachment(self, value):
        """Validate file attachment size and type"""
        if not value:
            return value
        ...
        if value.size > max_size:
            raise serializers.ValidationError(f"File size must be less than 5MB. Current size: {value.size / 1024 / 1024:.2f}MB")
        ...
        if file_extension not in allowed_extensions:
            raise serializers.ValidationError(f"File type '.{file_extension}' not allowed. Allowed types: {', '.join(allowed_extensions)}")
